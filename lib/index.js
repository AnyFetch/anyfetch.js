 ...
 */
var generateAdvancedMappings = function(extendedEndpoint, baseConfig, singleStepFunction) {
  var idMustBeNumeric = baseConfig.requireId;
  // Descriptors of the second-step calls available
  var configs = baseConfig.secondStep;
  ...
  var result = function(id, cb) {
    ...
    Object.keys(configs).forEach(function(name) {
      // Local copy
      var thisConfig = {};
      for (var i in configs[name]) {
        thisConfig[i] = configs[name][i];
      }

      // We've already taken care of those
      thisConfig.requireIdentifier = false;
      thisConfig.requireId = false;
      // Trick: "hardcode" id parameter in endpoint and proceed as usual
      thisConfig.endpoint = baseConfig.endpoint + thisConfig.endpoint;
      thisConfig.endpoint = thisConfig.endpoint.replace('{id}', id);
      ...
      secondStep[name] = function(){
        ...
        // Forward the call
        generateMapping(name, thisConfig).apply(this, arguments);
      };
 ...
 * If a key `secondStep` is specified, advanced mappings (two-steps)
 * are added as well.
 */
var generateMappings = function() {
  ...
  Object.keys(configuration.apiDescriptors).forEach(function(name) {
    ...
    if (config.secondStep) {
      mappingFunctions[name] = generateAdvancedMappings(name, config, mappingFunctions[name]);
    }
  });
  return mappingFunctions;
};
