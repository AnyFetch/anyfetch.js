...
var generateMapping = function(name, config) {
  ...
  return function apiMapping() {
    ...
    if(config.body) {
      ...
      if(util.isArray(body) && !config.noCheckBody) {
        for(key in body) {
    ...
    if(config.params) {
      ...
      if(util.isArray(params) && !config.noCheckParams) {
        for(key in params) {
    ...
    var r = request(this.apiHost)[config.method](endpoint)
      .query(params)
...
Anyfetch.prototype.setManagerHost = auth.setManagerHost;
Anyfetch.prototype.getAccessToken = auth.getAccessToken;
