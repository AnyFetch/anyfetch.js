 ...
 */
var encodeParentheses = function(string) {
...
module.exports = function(appId, appSecret, anyFetchServer, anyFetchFront) {
  ...
  this.API_ROOT = anyFetchServer || process.env.ANYFETCH_API_URL || 'http://api.anyfetch.com';
  ...
  */
  ...
  this.getAccessToken = function(code, redirect_uri, cb) {
    var params = {
      url: encodeParentheses(self.API_FRONT + self.ACCESSTOKEN_CREATION),
      form: {
        client_id: appId,
   ...
   *
   * @param {Object} datas to be sent to AnyFetch, following the documentation for API_ROOT/providers/documents.
  ...
  this.sendDocument = function(datas, hasFile, cb) {
    ...
    var params = {
      url: encodeParentheses(self.API_ROOT + self.DOCUMENT_CREATION),
      json: datas,
      headers: {
  ...
  this.sendFile = function(identifier, config, cb) {
    ...
    var params = {
      url: encodeParentheses(self.API_ROOT + self.DOCUMENT_CREATION + "/identifier/" + encodeURIComponent(identifier) + "/file"),
      headers: {
        'Authorization': 'token ' + self.accessToken
      }
    };
...
module.exports.debug = {
  ...
  encodeParentheses: encodeParentheses,
};
