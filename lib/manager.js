...
module.exports = function addManagerFunctions(AnyFetch) {
  ...
  AnyFetch.prototype.setManagerUrl = function(url) {
    if(!url) {
      throw new Error('Cannot set managerUrl to ' + url);
    }
    this.managerUrl = url;
  };
   ...
   * @param {String} REDIRECT_URI The redirect uri you set in manager.anyfetch.com
   * @param {String} code Code obtained during the first step of the oAuth flow
   * @param {function} cb(err, accessToken)
   */
  AnyFetch.getAccessToken = function(APP_ID, APP_SECRET, REDIRECT_URI, code, cb) {
    var body = {
      client_id: APP_ID,
    ...
    request(configuration.managerUrl)
      ...
      .expect(function(res) {
        if(!res.body || !res.body.access_token) {
          return new Error('Expected response to contain `access_token`, got ' + res.body);
        }
      })
