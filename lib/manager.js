...
module.exports = function addManagerFunctions(AnyFetch) {
   ...
   * @param {boolean} [trusted] Restrict to trusted providers only. Defaults to false.
   * @param {boolean} [featured] Restrict to featured providers only. Defaults to false.
   */
  AnyFetch.getAvailableProviders = function(trusted, featured, cb) {
    // Ommit `trusted` and `featured`
    if(!cb && (!featured && featured !== false)) {
      cb = trusted;
      trusted = false;
      featured = false;
    }
    ...
    request(configuration.managerUrl)
      ...
      .query({ trusted: trusted, featured: featured })
      .expect(200)
      .end(cb);
  };
