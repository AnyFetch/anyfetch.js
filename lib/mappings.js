...
var encodeUri = require('./helpers/encode-uri.js');
...
module.exports = function addMappings(Anyfetch) {
  ...
  var generateMapping = function(name, config) {
     ...
     * apiMapping(id, cb)
     * apiMapping(body, cb)
     * apiMapping(id, [body], cb)    (optional middle parameter)
     * apiMapping(id, [params], cb)  (optional middle parameter)
    ...
    return function apiMapping() {
      ...
      if(config.body) {
        ...
        isOmitted = isMongoId(body) || isString(body) || util.isArray(body) || isFunction(body);
      ...
      if(config.params) {
        ...
        isOmitted = isMongoId(params) || isString(params) || util.isArray(params) || isFunction(params);
      ...
      if(config.requireIdentifier) {
        var identifier = args.pop();
        // We allow `params` and / or `body` to be omitted
        identifier = identifier || params || body;
  ...
  var generateSubFunctionMappings = function(extendedEndpoint, baseConfig, singleStepFunction) {
     ...
     * - Single step: theEndpoint(123, [params], cb)
    ...
    return function(id, params, cb) {
      ...
      if(isFunction(cb)) {
        ...
        if(baseConfig.params) {
          return singleStepFunction.call(this, id, params, cb);
        }
        else {
          return singleStepFunction.call(this, id, cb);
        }
      }
  ...
  var addPostFileFunctions = function() {
    ...
    var addPostFileFunction = function(endpointToExtend) {
      ...
      Anyfetch.prototype[endpointToExtend] = function(id, params, cb) {
        if(!cb) {
          cb = params;
          params = {};
        }

        var ret = previousVersion.call(this, id, params, cb);
        // Two-step call syntax
